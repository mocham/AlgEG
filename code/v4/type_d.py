"""Verify the stable Type D adjacent-height graph families."""
...
from algebra import jacobian_matrix
from graphs import (
    bipartite_layers,
...
from roots import positive_roots, root_string, root_tuple, subtract
...
def expected_core(rank, height):
    """Return K_(rank,m), or the empty graph."""
...
def verify_full_layers(max_rank):
    ...
    for rank in range(4, max_rank + 1):
        ...
        for height in range(1, maximum_height):
            ...
            if actual != expected:
                raise AssertionError(f"D{rank} height {height}: unexpected graph")
            if actual:
                nonempty_cores += 1
        ...
        for height, layer in sorted(layers.items()):
            ...
            for size in range(len(targets) + 1):
                for selected in combinations(targets, size):
                    rank_tested += 1
                    remaining, _trace = contract_source_leaves(
                        induced_bipartite(layer, sources, selected))
                    predicted_nonempty = (
                        bool(expected) and expected_targets <= set(selected))
                    if bool(remaining) != predicted_nonempty:
                        raise AssertionError(
                            f"D{rank} height {height}: subset criterion failed")
                    if remaining and remaining != expected:
                        raise AssertionError(
                            f"D{rank} height {height}: unexpected induced graph")
                    if remaining:
                        rank_nonempty += 1
                        signatures.add((height, tuple(remaining.items())))
    ...
    for core in cores:
        for source, targets in core.items():
            for target in targets:
                ...
                if not coefficient:
                    raise AssertionError(
                        f"D{rank}: missing structure constant {key}")
...
def expected_determinant_variables(rank, height):
    ...
    return {
        "x" + root_string(tuple(
            int(position == index) for position in range(1, rank + 1)))
...
def verify_determinants(max_rank):
    ...
    for rank in range(4, max_rank + 1):
        ...
        constants = limited_structure_constants(
            rank, [core for _height, core in cores])
        rank_records = []
        for height, core in cores:
            jacobian, _sources, _targets = jacobian_matrix(core, constants)
            if jacobian.nrows() != jacobian.ncols():
                raise AssertionError(f"D{rank} height {height}: matrix is not square")
            determinant = jacobian.det()
            terms = determinant.dict()
            if len(terms) != 1:
                raise AssertionError(
                    f"D{rank} height {height}: determinant is not one monomial")
            exponents, coefficient = next(iter(terms.items()))
            coefficient = int(coefficient)
            coefficients.add(coefficient)
            if abs(coefficient) != 2:
                raise AssertionError(
                    f"D{rank} height {height}: coefficient is not +/-2")
            actual_variables = {
                name
...
def verify_type_d(
    ...
    subset_max=DEFAULT_SUBSET_MAX,
    determinant_max=DEFAULT_DETERMINANT_MAX,
):
    """Run the three finite Type D checks."""
    ...
    return {
        "implementation_sha256": {
            ...
            for name in ("algebra.py", "graphs.py", "roots.py", "type_d.py")
        },
        "stable_family": {
            "nonempty_heights": "h=2m with 1<=m<=floor((n-2)/2)",
            "remaining_graph": "directed C6 joined at one target to directed P_(2m-1)",
            "other_heights": "empty after degree-one source removal",
